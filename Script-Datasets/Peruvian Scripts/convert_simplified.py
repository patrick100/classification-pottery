...
path = './Peruvian_Dataset_manifold/'
path1 = './Peruvian_Dataset_simplified/'
...
def convert_txt(path,categories,DataGroup):
	...
	for cat  in categories:
		...
		for file_index,file in enumerate(files):
			...
			command = " ".join(["./simplify","-i",path + cat + '/'+DataGroup+'/' + file,"-o",path1 + cat +'/'+DataGroup+'/' + file,"-m -c 1e-2 -f 10000 -r 0.2"])
