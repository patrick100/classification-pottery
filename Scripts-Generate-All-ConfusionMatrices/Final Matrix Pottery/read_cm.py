...
def plot_confusion_matrix(NAME_FILE,ax, classes,
	...
	for i in range(cm.shape[0]):
	    ...
	    for j in range(cm.shape[1]):
	        if(cm[i, j] == 0):
	            ax.text(j, i, format(cm[i, j], '.0f'),ha="center", va="center",color="black" if cm[i, j] > thresh else "white",fontsize=4)
	        else:
	            ax.text(j, i, cm[i, j],ha="center", va="center",color="black" if cm[i, j] > thresh else "white",fontsize=4)
...
NAME_FILE10 = 'MVCNN_geodesic_max1_47';
